...
def load_package_paths():
    ...
    if not os.path.exists(RCMUX_LIB_LOCATION):
        raise ImportError(f"Cannot find rcmux library!")
...
class Runner:
    ...
    def __init__(self):
        ...
        self.RCMUX_CLIENT = RcMuxClient()

        self.USER_PIPE_NAME = PipeName((PipeType.INPUT, "start-button", "starter"), self.PIPE_DIRECTORY)
        self.RCMUX_CLIENT.open_pipe(self.USER_PIPE_NAME, delete=True, create=True)
    ...
    def __start(self, params):
        ...
        if self.STATE == State.ready:
            ...
            self.RCMUX_CLIENT.write(self.USER_PIPE_NAME, start_args.encode("utf-8"))
    ...
    def run(self):
        self.RUNNING = True
        while (1):
            b = self.RCMUX_CLIENT.read(self.FLASK_PIPE_NAME)
